...
- **Project Path**: `C:\Codex\PowerApps\Quota Validator\`
...
- **SD Quota File** (.xlsx): `SD IC`와 `SD MGRs` 시트만 읽음
- **Reference File** (.xlsx): Sales Compensation Report (Daily)
- 선택한 submission month 기준으로 SD 레코드만 검증
...
- SD V3는 `Component 1`, `Component 2` 월별 quota를 사용
- SD parser는 `SD IC`와 `SD MGRs`의 고정 구조를 기준으로 읽음
- sensitivity / 암호화가 적용된 workbook은 브라우저에서 직접 파싱되지 않을 수 있음
...
├── quota-validation-powerapp/      # Power Apps Code App 배포 폴더
├── src/
...
│   │   └── SdValidationTab.tsx     # SD 전용 업로드/검증 탭
│   └── utils/
│       ├── excelParser.ts          # Excel 파싱, 동적 컬럼 감지
│       ├── sdExcelParser.ts        # SD IC / SD MGRs 파싱
...
### 6. SD 시트 파싱
- `SD IC`와 `SD MGRs`는 row 5 헤더, row 6 이후 데이터 구조를 사용
- `SD IC`의 예시 행은 제외
- Component 1 월별 quota는 `JUL..JUN` 블록(26~37열), Component 2는 `JUL..JUN` 블록(64~75열)을 읽음
- SD 결과 화면의 V3 라벨은 `Y1` 대신 `Component 1`, `C2:Y1` 대신 `Component 2`로 표시
...
- Power Apps 배포는 `quota-validation-powerapp/` 폴더에서 수행
- 해당 폴더의 `power.config.json`이 Power Apps app/environment 연결 정보를 보유
