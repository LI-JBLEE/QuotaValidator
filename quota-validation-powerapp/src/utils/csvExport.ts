...
export function exportV3Results(
  results: ValidationResults,
  labels: V3DisplayLabels = DEFAULT_V3_DISPLAY_LABELS,
) {
  ...
  const primaryHeaders = monthKeys.map((m) => `${labels.primary}-${m}`);
  const primarySecondaryHeaders = anyHasY2Y3
    ? monthKeys.map((m) => `${labels.primarySecondary}-${m}`)
    : [];
  const secondaryHeaders = anyHasComp2
    ? monthKeys.map((m) => `${labels.secondary}-${m}`)
    : [];
  const secondarySecondaryHeaders = anyHasComp2Y2Y3
    ? monthKeys.map((m) => `${labels.secondarySecondary}-${m}`)
    : [];

  const rows: string[] = [];
  rows.push(toCsvRow([
    'Sheet', 'Row', 'EID', 'Name', 'Dual Metric', 'Quota Start Date', 'Half',
    `Missing (${labels.primary})`, ...primaryHeaders,
    ...(anyHasY2Y3 ? [`Missing (${labels.primarySecondary})`, ...primarySecondaryHeaders] : []),
    ...(anyHasComp2 ? [`Missing (${labels.secondary})`, ...secondaryHeaders] : []),
    ...(anyHasComp2Y2Y3
      ? [`Missing (${labels.secondarySecondary})`, ...secondarySecondaryHeaders]
      : []),
  ]));
...
export function exportAllResults(
  ...
  for (const r of results.v3) {
    ...
    if (r.missingMonths.length > 0) details.push(`${labels.primary}: ${r.missingMonths.join(', ')}`);
    if (r.missingMonthsY2Y3.length > 0) {
      details.push(`${labels.primarySecondary}: ${r.missingMonthsY2Y3.join(', ')}`);
    }
    if (r.missingMonthsComp2.length > 0) {
      details.push(`${labels.secondary}: ${r.missingMonthsComp2.join(', ')}`);
    }
    if (r.missingMonthsComp2Y2Y3.length > 0) {
      details.push(`${labels.secondarySecondary}: ${r.missingMonthsComp2Y2Y3.join(', ')}`);
    }
    rows.push(toCsvRow([
